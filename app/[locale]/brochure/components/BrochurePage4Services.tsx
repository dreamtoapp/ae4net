...
export function BrochurePage4Services() {
  ...
  return (
    <div className="pdf-page p-12 flex flex-col bg-gradient-to-br from-brand-light-gray to-muted relative overflow-hidden">
      ...
      <div className="page-number relative z-10">{tc('page')} 4 {tc('of')} 9</div>
      ...
      <div className="text-center mb-10 relative z-10">
        ...
        <h2 className="text-4xl font-black text-foreground mb-2">{t('services.title')}</h2>
        <p className="text-base text-muted-foreground font-light">{t('services.subtitle')}</p>
      </div>

      <div className="space-y-6 flex-1 relative z-10">
        {services.slice(0, 2).map((service, idx) => (
          <div key={service.id} className="relative bg-background p-8 rounded-2xl shadow-xl border-l-4 border-primary hover:shadow-2xl transition-shadow">
            <div className="absolute top-0 right-0 w-32 h-32 bg-gradient-to-br from-primary/10 to-transparent rounded-bl-full" />
            <div className="relative">
              <div className="flex items-start justify-between mb-4">
                <div className="flex-1">
                  <div className="inline-block px-3 py-1 bg-primary/10 text-primary text-xs font-bold rounded-full mb-3 uppercase tracking-wide">
                    {tb('service')} {idx + 1}
                  </div>
                  ...
                  <div className="flex items-baseline gap-2 mb-2">
                    <div className="text-4xl font-black bg-gradient-to-r from-primary to-brand-navy bg-clip-text text-transparent">
                      {service.stats.count}
                    </div>
